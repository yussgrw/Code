...
void msort(int l, int r, bool (*_comp) (int*, int*), int mode) {
	if(l==r) {
	} else if(r-l==1) {
		if(!_comp(&a[l],&a[r])) {
			int tmp=a[l];
		...
		if(total<MAX) {
			thread th1,th2;
			th1=thread(msort,l,mid,_comp,1);
			th2=thread(msort,mid+1,r,_comp,1);
			th1.join();
			th2.join();
			total ++;
			total ++;
		} else {
			msort(1,mid,_comp,0);msort(mid+1,r,_comp,0);
		}
